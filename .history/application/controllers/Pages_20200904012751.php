            ...
            if(isset($this->session->userid)){
                ...
                $this->load->view('templates/footer');
                    ...
                    if(!empty($_FILES['profilepic']['name'])){
                        $_FILES['userfile']['name']= time().rand(1,988).'.'.substr(strrchr($_FILES['profilepic']['name'],'.'),1);	
                        $_FILES['userfile']['tmp_name']= $_FILES['profilepic']['tmp_name'];
                        $_FILES['userfile']['type']= $_FILES['profilepic']['type'];
                        $_FILES['userfile']['error']= $_FILES['profilepic']['error'];
                        $_FILES['userfile']['size']= $_FILES['profilepic']['size'];
                        
                        $save['profilepic'] = $_FILES['userfile']['name'];
                        
                        move_uploaded_file($_FILES['userfile']['tmp_name'],BASEPATH.'../assets/admin/uploads/profilepics/'.$_FILES['userfile']['name']);
    
                        if(!empty($this->input->post('old_profilepic'))){
                            if(file_exists(BASEPATH.'../assets/admin/uploads/profilepics/'.$this->input->post('old_profilepic')) && $flag)
                            unlink(BASEPATH.'../assets/admin/uploads/profilepics/'.$this->input->post('old_profilepic'));
                        }
