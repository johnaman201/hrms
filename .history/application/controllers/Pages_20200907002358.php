...
public function postadd($id = false){
    ...
    if(isset($this->session->userid)){
        ...
        $data['position'] = $this->DB_model->get_position($id);
        //$data['designations'] = $this->DB_model->get_designations();
        //$data['positions'] = $this->DB_model->get_positions();
        //echo '<pre>'.print_r($data['dept']);die;
        //$data['login_user'] = 'User Login';
        ...
        $this->load->view('templates/footer');
            ...
            $data = array(
                ...
                'positionstatus' => $this->input->post('positionstatus')                    
            );
            //echo '<pre>'.print_r($id);die;
            if(!$id){
                ...
                if($result){
                    ...
                    $this->session->set_flashdata('post_add_success','New Department Added');
                    $this->session->keep_flashdata('post_add_success');

                    redirect(site_url('positions'));
                }
                else{
                    $this->session->set_flashdata('post_add_error','New Department not Added');
                ...
                if($result){
                    ...
                    $this->session->set_flashdata('post_update_success','Department Data Updated');
                    $this->session->keep_flashdata('post_update_success');

                    redirect(site_url('positions'));
                }
                else{
                    $this->session->set_flashdata('post_update_error','Department Data not Updated');
